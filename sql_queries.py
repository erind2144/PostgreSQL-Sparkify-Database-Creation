...
songplay_table_create = ("CREATE TABLE IF NOT EXISTS songplays (songplay_id \
    serial PRIMARY KEY, start_time timestamp, user_id int, level varchar, \
    song_id varchar, artist_id varchar, session_id int, location varchar, \
    user_agent varchar);")

user_table_create = ("CREATE TABLE IF NOT EXISTS users (user_id int PRIMARY \
    KEY, first_name varchar, last_name varchar, gender varchar, level \
    varchar);")

song_table_create = ("CREATE TABLE IF NOT EXISTS songs (song_id varchar \
    PRIMARY KEY, title varchar, artist_id varchar, year int, duration float);")

artist_table_create = ("CREATE TABLE IF NOT EXISTS artists (artist_id varchar \
    PRIMARY KEY, name varchar, location varchar, latitude float, longitude \
    float);")

time_table_create = ("CREATE TABLE IF NOT EXISTS time (start_time timestamp \
    PRIMARY KEY, hour int, day int, week int, month int, year int, weekday \
    varchar);")
...
songplay_table_insert = ("INSERT INTO songplays (start_time, user_id, level, \
    song_id, artist_id, session_id, location, user_agent) VALUES (%s, %s, %s, \
    %s, %s, %s, %s, %s)")

user_table_insert = ("INSERT INTO users (user_id, first_name, last_name, \
    gender, level) VALUES (%s, %s, %s, %s, %s) ON CONFLICT (user_id) DO \
...
artist_table_insert = ("INSERT INTO artists (artist_id, name, location, \
    latitude, longitude) VALUES (%s, %s, %s, %s, %s) ON CONFLICT (artist_id) \
    DO UPDATE SET location = EXCLUDED.location, latitude = \
    EXCLUDED.latitude, longitude = EXCLUDED.longitude")

time_table_insert = ("INSERT INTO time (start_time, hour, day, week, month, \
    year, weekday) VALUES (%s, %s, %s, %s, %s, %s, %s) ON CONFLICT \
    (start_time) DO NOTHING")


# FIND SONGS

song_select = ("SELECT songs.song_id, songs.artist_id FROM (songs INNER JOIN \
    artists ON songs.artist_id = artists.artist_id) WHERE songs.title = %s \
